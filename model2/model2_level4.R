...
# Read the data
cats <- read_data(type = 2)
# Prepare dataset
df <- prepare_data(cats, level = level, minimum = 10, type = 2)
summary_data(df, cats)

# Tokenize: Break labels into sequence of words
words <- h2o_tokenize(df$product, dropnumwords = FALSE, minchar = 1, stop_words = STOP_WORDS)
